...
class Laporan_permasalahan extends CI_Controller{
	...
	public function index($lanud_id){
		...
		//echo "select a.kotama,b.lanud,c.lokasi,c.nilai,d.permasalahan,g.bmn,g.bangunan,g.tanah,g.sewa,d.hasil_klarifikasi,d.tindak_lanjut from kotama a,m_lanud b,data_aset c,permasalahan d,pemanfaatan f,detil_pemanfaatan g where b.id='".$x[3]."' and b.kotama=a.id and c.lanud=b.id and d.lanud=b.id and d.simak=f.simak and f.id=g.t_pemanfaatan".$q_tahun." order by d.id desc,g.id desc";die();
		// 20160608 $data['data'] = $this->db->query("select a.kotama,b.lanud,c.lokasi,c.nilai,d.permasalahan,g.bmn,g.bangunan,g.tanah,g.sewa,d.hasil_klarifikasi,d.tindak_lanjut from kotama a,m_lanud b,data_aset c,permasalahan d,pemanfaatan f,detil_pemanfaatan g where b.id='".$x[3]."' and b.kotama=a.id and c.lanud=b.id and d.lanud=b.id and d.simak=f.simak and f.id=g.t_pemanfaatan".$q_tahun." order by d.id desc,g.id desc")->result();
		$data['data'] = $this->db->query("select a.permasalahan,a.hasil_klarifikasi,a.tindak_lanjut,b.lanud,c.kotama,d.lokasi,d.nilai,f.bangunan,f.tanah,f.sewa,f.bmn from permasalahan a,m_lanud b,kotama c,data_aset d,pemanfaatan e,detil_pemanfaatan f where a.lanud='".$x[3]."' and a.lanud=b.id and b.kotama=c.id and a.simak=d.simak and a.lanud=d.lanud and e.lanud=a.lanud and e.simak=a.simak and f.t_pemanfaatan=e.id")->result();
